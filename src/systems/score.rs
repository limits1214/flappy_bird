...
pub fn scoreing_helper<T: Component>(
    ...
    if let Ok((entity, transform, children)) = query.get_single() {
        ...
        let mut x_offset = 0.;
        let vstr = get_score_entitiy_vec(commands, fb_assets, is_large, offset, number, &mut x_offset);
        commands.entity(entity).push_children(vstr.as_slice());
...
pub fn get_score_entitiy_vec(commands: &mut Commands, fb_assets: &Res<FlappyBirdAssets>, is_large: bool, offset: f32, number: u32, x_offset: &mut f32) -> Vec<Entity> {
